...
exports = Class(StackView, function(supr) {
    ...
    this.hideTransitionView = new ImageView({
    ...
    this.showTransitionView = new ImageView({
        image: this.transition,
        width: this.transition.getWidth(),
        height: this.transition.getHeight() + 5,
        x: (device.screen.width - this.transition.getWidth())/2 ,
    ...
    this._hide = function (view, dontAnimate, reverse) {
            ...
            if (!dontAnimate) {
                this.hideTransitionView.updateOpts({
                    scale: 120
                });
                view.addSubview(this.hideTransitionView);
                this.getInput().blockEvents = true;
                animate(view)
                        .now(bind(this, function(){
                            animate(this.hideTransitionView)
                                .then({scale: 1}, 500)
    ...
    this._show = function (view, dontAnimate, reverse) {
		...
		if (!dontAnimate) {
                        this.getInput().blockEvents = true;
                        this.showTransitionView.updateOpts({
                            scale: 1
                        });
                        view.addSubview(this.showTransitionView);
			...
			animate(view)
                            ...
                            .then({opacity: 1}, 0)
                            .then(bind(this, function(){
                                animate(this.showTransitionView)
                                    .then({scale: 120}, 1500)
                                    .then(bind(view, 'publish', 'ViewDidAppear'))
                                    .then(bind(this,function(){
                                        view.removeSubview(this.showTransitionView);
                                        this.getInput().blockEvents = false;
                                    }))
                             }));
		} else {
